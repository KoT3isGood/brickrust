...
{
    let sig = lookup("UBrickStatics::GetProjectVersion",sig!("48 8b 93 18 01 00 00 48 8b cf 48 81 c2 b8 00 00 00")).sub(0x2B);
    GetProjectVersion_ptr = Some(transmute(sig));
    
    let sig = lookup("UBrickStatics::IsModdedAsset",sig!("48 83 ec 58 48 85 c9 0f 84 ?? ?? ?? ??"));
